...
procedure TMainForm.FormCreate(Sender: TObject);//загрузка шрифтов
var filepath:string;
begin
...
procedure TMainForm.FormDestroy(Sender: TObject);//выгрузка шрифтов
var filepath:string;
begin
filepath:=extractfilepath(application.ExeName);
...
if not Assigned(ArrayForm) then
    ArrayForm:= TArrayForm.Create(Application);  // Создать форму, если она еще не создана

ArrayForm.SetMenuID(1);  // Установить Menu_ID
ArrayForm.show;
MainForm.hide;
end;
...
begin
  ...
  ArrayForm.SetMenuID(2);  // Установить Menu_ID
  ArrayForm.show;
  MainForm.hide;
end;
